 ...
 * @version 1.0
 */
if ( ! class_exists( 'MP_Gateway_myCRED' ) && class_exists( 'MP_Gateway_API' ) ) :
	class MP_Gateway_myCRED extends MP_Gateway_API {
		...
		function on_creation() {
			...
			$this->public_name           = MYCRED_DEFAULT_LABEL;

			if ( isset( $settings['gateways']['mycred']['name'] ) && ! empty( $settings['gateways']['mycred']['name'] ) )
				$this->public_name = $settings['gateways']['mycred']['name'];

			$this->method_img_url        = plugins_url( 'assets/images/mycred-token-icon.png', MYCRED_MARKETPRESS );
		...
		function get_cart_total( $cart = NULL ) {
			...
			if ( $this->use_exchange() )
				$cost = $this->mycred->apply_exchange_rate( $total, $settings['gateways']['mycred']['exchange'] );
			else
				$cost = $this->mycred->number( $total );

			return apply_filters( 'mycred_marketpress_order_cost', $cost, $total, $settings, $cart, $this );
...
<table style="width:100%;">
	...
	<tr>
		...
		function confirm_payment_form( $cart, $shipping_info ) {
			...
			$table    = '<table class="mycred-cart-cost"><thead><tr><th>' . __( 'Payment', 'mycred_market' ) . '</th></tr></thead>';
			if ( $balance-$total < 0 ) {

				$message = $this->mycred->template_tags_user( $settings['gateways']['mycred']['lowfunds'], false, wp_get_current_user() );
				$table .= '<tr><td id="mp-mycred-cost" style="color: red; font-weight: bold;"><p>' . $message . ' <a href="' . mp_checkout_step_url( 'checkout' ) . '">' . __( 'Go Back', 'mycred_market' ) . '</a></td></tr>';

			}

			else
				$table .= '<tr><td id="mp-mycred-cost" class="mycred-ok">' . $this->mycred->format_creds( $total ) . ' ' . __( 'will be deducted from your account.', 'mycred_market' ) . '</td></tr>';
		...
		function process_payment( $cart, $billing_info, $shipping_info ) {
			...
			if ( $this->mycred->exclude_user( $user_id ) )
				$mp->cart_checkout_error(
					sprintf( __( 'Sorry, but you can not use this gateway as your account is excluded. Please <a href="%s">select a different payment method</a>.', 'mycred_market' ), mp_checkout_step_url( 'checkout' ) )
				);
			...
			if ( $balance <= $this->mycred->zero() || $balance-$total < $this->mycred->zero() ) {
				...
				$mp->cart_checkout_error(
					$insolvent . ' <a href="' . mp_checkout_step_url( 'checkout' ) . '">' . __( 'Go Back', 'mycred_market' ) . '</a>'
				);
				return;
			...
			$payment_info['method']               = MYCRED_DEFAULT_LABEL;
		...
		function gateway_settings_box( $settings ) {
			...
			$settings['gateways']['mycred'] = shortcode_atts( array(
				'name'                 => $name . ' ' . $mycred->template_tags_general( __( '%_singular% Balance', 'mycred_market' ) ),
				'logo'                 => $this->method_button_img_url,
				'type'                 => MYCRED_DEFAULT_TYPE_KEY,
				'log_template'         => __( 'Payment for Order: #%order_id%', 'mycred_market' ),
				'exchange'             => 1,
				'profit_share_percent' => 0,
				'profit_share_log'     => __( 'Product Sale: %post_title%', 'mycred_market' ),
				'instructions'         => __( 'Pay using your account balance.', 'mycred_market' ),
				'confirmation'         => __( 'TOTAL amount has been deducted from your account. Your current balance is: %balance_f%', 'mycred_market' ),
				'lowfunds'             => __( 'Insufficient funds.', 'mycred' ),
				'visitors'             => __( 'You must be logged in to pay with %_plural%. Please <a href="%login_url_here%">login</a>.', 'mycred_market' ),
				'email'                => $settings['email']['new_order_txt']
			), ( ( array_key_exists( 'mycred', $settings['gateways'] ) ) ? $settings['gateways']['mycred'] : array() ) );

?>
<div id="mp_mycred_payments" class="postbox mp-pages-msgs">
	<h3 class="handle"><span><?php printf( __( '%s Settings', 'mycred_market' ), MYCRED_DEFAULT_LABEL ); ?></span></h3>
	<div class="inside">
		<span class="description"><?php echo sprintf( __( 'Let your users pay for items in their shopping cart using their %s Account. Note! This gateway requires your users to be logged in when making a purchase!', 'mycred_market' ), $name ); ?></span>
		<table class="form-table">
			<tr>
				<th scope="row"><label for="mycred-method-name"><?php _e( 'Method Name', 'mycred_market' ); ?></label></th>
				<td>
					<span class="description"><?php _e( 'Enter a public name for this payment method that is displayed to users - No HTML', 'mycred_market' ); ?></span>
			...
			<tr>
				<th scope="row"><label for="mycred-method-logo"><?php _e( 'Gateway Logo URL', 'mycred_market' ); ?></label></th>
				<td>
			...
			<tr>
				<th scope="row"><label for="mycred-method-type"><?php _e( 'Point Type', 'mycred_market' ); ?></label></th>
				<td>
					<?php mycred_types_select_from_dropdown( 'mp[gateways][mycred][type]', 'mycred-method-type', $settings['gateways']['mycred']['type'] ); ?>
				</td>
			</tr>
			<tr>
				<th scope="row"><label for="mycred-log-template"><?php _e( 'Log Template', 'mycred_market' ); ?></label></th>
				<td>
...
<?php
				...
				if ( $this->use_exchange() ) :
					...
					$exchange_desc = __( 'How much is 1 %_singular% worth in %currency%?', 'mycred_market' );
...
?>
			<tr>
				<th scope="row"><label for="mycred-exchange-rate"><?php _e( 'Exchange Rate', 'mycred_market' ); ?></label></th>
				<td>
...
?>
			...
			<tr>
				<th scope="row"><label for="mycred-profit-sharing"><?php _e( 'Percentage', 'mycred_market' ); ?></label></th>
				<td>
					<span class="description"><?php _e( 'Option to share sales with the product owner. Use zero to disable.', 'mycred_market' ); ?></span>
			...
			<tr>
				<th scope="row"><label for="mycred-profit-sharing-log"><?php _e( 'Log Template', 'mycred_market' ); ?></label></th>
				<td>
			...
			<tr>
				<th scope="row"><label for="mycred-lowfunds"><?php _e( 'Insufficient Funds', 'mycred_market' ); ?></label></th>
				<td>
					<span class="description"><?php _e( 'Message to show when the user can not use this gateway.', 'mycred_market' ); ?></span>
			...
			<tr>
				<th scope="row"><label for="mycred-visitors"><?php _e( 'Visitors', 'mycred_market' ); ?></label></th>
				<td>
					<span class="description"><?php _e( 'Message to show to buyers that are not logged in.', 'mycred_market' ); ?></span>
			...
			<tr>
				<th scope="row"><label for="mycred-instructions"><?php _e( 'User Instructions', 'mycred_market' ); ?></label></th>
				<td>
					<span class="description"><?php _e( 'Information to show users before payment.', 'mycred_market' ); ?></span>
			...
			<tr>
				<th scope="row"><label for="mycred-confirmation"><?php _e( 'Confirmation Information', 'mycred_market' ); ?></label></th>
				<td>
					<span class="description"><?php _e( 'Information to display on the order confirmation page. - HTML allowed', 'mycred_market' ); ?></span>
...
<?php
		...
		function process_gateway_settings( $settings ) {
			...
			if ( ! array_key_exists( 'mycred', $settings['gateways'] ) ) return $settings;
